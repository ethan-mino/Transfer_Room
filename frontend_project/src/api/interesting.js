...
async function insertInterest(interestingInfo, success, fail) {
  ...
  await interApi
    .post(`/user/interesting-area`, JSON.stringify(interestingInfo))
    .then(success)
    .catch(fail);
}
...
async function delInterest(id, success, fail) {
  interApi.defaults.headers["Authorization"] =
    sessionStorage.getItem("access-token");
  await interApi
    .delete(`/user/interesting-area/${id}`)
    .then(success)
    .catch(fail);
}

export { insertInterest, getInterest, delInterest };
