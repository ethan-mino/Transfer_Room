      ...
      <div class="content">
        ...
        <div class="transfer_board" style="width: 100%; padding: 20px">
          <div class="board_infos">
            ...
            <div>
              <span class="room_type">{{ boardInfo.roomType }}</span> /
              <span class="contract_type">{{ boardInfo.contractType }}</span>
            </div>
            ...
            <div
              class="tb_content"
              style="font-size: 14px; padding: 20px 0px; border-bottom: 1px solid lightgray"
              v-html="boardInfo.transferContent.replace(/\n/gi, '</br>')"
            >
          ...
          <div style="margin-top: 15px">
            <a v-for="(file, idx) in boardInfo.attachedFileList" :key="idx">
              <a :href="'http://localhost:8080/transfer-board/file/' + file.fileId">{{
        ...
        <div class="btn-wrap" style="margin-bottom: 20px">
          <button class="list" @click="listBtn">목록으로.</button>
          <button class="add" @click="registBtn" v-if="boardInfo.transfererId != currentUserId">양도 요청</button>
        </div>
      </div>
    </div>
  </div>
...
import { getTransferBoardById, transferRequest } from "@/api/transferBoard";
import { getUser, getCurrnetUser } from "@/api/member";
  ...
  methods: {
    ...
    registBtn: async function () {
      await transferRequest(
        this.boardInfo.transferBoardId,
        ({ data }) => {
          console.log(data);
        },
        (error) => {
